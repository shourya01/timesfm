...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        for itm in (t:=tqdm(dl)):
            ...
            t.set_description(f"Step {steps+1} on epoch {epoch+1} took {(end-start):.3f}s. MSE loss: {mse_loss:.5f}, MAE loss: {mae_loss:.5f}. Expected time for this epoch: {int((end-start)*train_len)}s")
        # test
        ...
        for itm in dl_test:
            ...
            test_mae += F.l1_loss(out,y,reduction='mean')
        if epoch == 0:
            losses_test = pd.DataFrame({'mse_loss':[test_mse/test_len],'mae_loss':[test_mae/test_len]})
            losses_test.index.name = 'epoch'
        else:
            losses_test.loc[epoch].append({'mse_loss':test_mse/test_len,'mae_loss':test_mae/test_len})
